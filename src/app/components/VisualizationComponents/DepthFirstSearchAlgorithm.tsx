...
import { GraphSearchVisualizationInterface } from "@/app/interfaces/GraphSearchVisualizationInterface";
...
const DepthFirstSearchAlgorithmVisualizer = () =>{
    ...
    const animationsRef = useRef<GraphSearchVisualizationInterface[]>([]);
    const [animations,setAnimations] = useState<GraphSearchVisualizationInterface[]>([]);
   ...
   const visualizeGraph = () =>{
    ...
    animations = depthFirstSearch(graph,startNode);
    if(animationsRef.current.length>0)
    {
        setAnimations([]);
        resetNodesAndPaths();
...
export default DepthFirstSearchAlgorithmVisualizer;